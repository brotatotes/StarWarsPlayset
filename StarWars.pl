...
roles_relation(stormtrooper_unit_1/stormtrooper_unit_2).
...
need(escape_from_bounty_hunters_or_pay_off_debts).
need(realize_potential_as_jedi).
need(hunt_for_bounty).
need(deliver_secret_message_to_rebels).
need(make_quick_and_dirty_fortune).
need(convert_everyone_to_dark_side).
need(convert_your_son_to_dark_side).
